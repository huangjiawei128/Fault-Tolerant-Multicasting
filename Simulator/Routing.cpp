...
NodeInfo *Routing::forward(Message &s) {
    ...
    return forward((*cube)[s.routpath[0].node], (*cube)[s.dst]);
}

int Routing::chkWrplnk(CubeNode *cur, CubeNode *dst) {
    int curx = cur->x;
    int cury = cur->y;
...
NodeInfo *Routing::forward(CubeNode *cur, CubeNode *dst) {
...
bool Routing::checkBuffer(Buffer *buff1, int &chn, Buffer *&record) {
    bool k = false;
    if (buff1->link_used == true) {
        return k;
    }

    if (chn == R1) {
        if (buff1->r1 >= MESSAGE_LENGTH) {
            k = true;
            buff1->link_used = true;
        }
    ...
    /*else{
        ...
        else if(buff1->r2 >= MESSAGE_LENGTH){
            chn = R2;
            k = true;
            buff1->link_used = true;
        }
    ...
    if (k) {
        record = buff1;
        buff1->bufferMin(chn, MESSAGE_LENGTH);
    }
    return k;
}
...
NodeInfo *Routing::noWrapLinkrt(CubeNode *cur, CubeNode *dst) {  //routing without wraplink.
    assert(cur && dst && (cur != dst));
    int curx = cur->x;
    int cury = cur->y;
...
NodeInfo *Routing::oneWrapLinkrt(CubeNode *cur, CubeNode *dst) {  //routing one wraplink.
    assert(cur && dst && (cur != dst));
    int curx = cur->x;
    int cury = cur->y;
...
NodeInfo *Routing::twoWrapLinkrt(CubeNode *cur, CubeNode *dst) {
    assert(cur && dst && (cur != dst));
    int curx = cur->x;
    int cury = cur->y;
